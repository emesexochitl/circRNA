...
class iso_set():
    ...
    def save_files(self):
        with  open('average_genomic_exon_len_num.txt', 'a') as f:
            f.write("%s\t%.3f\t%.3f\n" % (self.iso_id, self.iso_num, self.av_len[0]))


try:
    os.remove("average_genomic_exon_len_num.txt")
except:
    pass
...
exonlen_file = "Arabidopsis_thaliana.TAIR10.34_isoform_exon_coordinates_det.txt"
...
ids=[item[1] for item in exonlen_list]
