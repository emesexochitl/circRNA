...
class iso_set():
    ...
    def __init__(self,iso_id,iso_num):
        ...
        self.total_len = sum(self.all_len)
        self.av_len = [sum(self.all_len)/self.iso_num]

        self.save_files()

    def save_files(self):
        with  open('average_exp1_intron_len_num2.txt', 'a') as f:
            f.write("%s\t%d\t%d\t%.3f\n" % (self.iso_id, self.iso_num, self.total_len, self.av_len[0]))


try:
    os.remove("average_exp1_intron_len_num2.txt")
except:
    pass
...
exonlen_file = "../references/Arabidopsis_thaliana.TAIR10.34_isoform_intron.saf"
...
ids=[item[0] for item in exonlen_list if item[0] in pre_ids]
